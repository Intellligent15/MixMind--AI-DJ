...
from app.services.llm.digitalocean import DigitalOceanProvider, DEFAULT_MODEL


@pytest.mark.asyncio
async def test_do_provider_caching():
...
@pytest.mark.asyncio
async def test_do_provider_generation_unwraps_plan_key():
...
@pytest.mark.asyncio
async def test_do_provider_accepts_raw_list():
...
@pytest.mark.asyncio
async def test_do_provider_recovers_from_stray_leading_brace():
    """gpt-oss sometimes prepends a stray '{' before the real
    {'plan': [...]} object. The tolerant parser must still recover it."""
    mock_storage = AsyncMock()
    mock_storage.exists.return_value = False

    dummy_plan = [{"tool": "set_transition_window", "from_song_time_start": 1.0,
                   "to_song_time_start": 2.0, "duration_bars": 8}]
    # Note the junk leading brace + newline before the valid object.
    malformed = "{\n" + json.dumps({"plan": dummy_plan})

    mock_response = MagicMock()
    mock_response.choices = [MagicMock(message=MagicMock(content=malformed))]
    mock_client = MagicMock()
    mock_client.chat.completions.create.return_value = mock_response
...
@pytest.mark.asyncio
async def test_do_provider_invalid_json_raises():
    mock_storage = AsyncMock()
    mock_storage.exists.return_value = False
...
@pytest.mark.asyncio
async def test_do_provider_unexpected_shape_raises():
