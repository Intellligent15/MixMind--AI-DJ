...
**Phase 8 + 8.5 complete** — queue-wide FLAC stitching + Genius lyric fetch + DTW alignment + vocal safety. `POST /api/queues/{id}/stitch` dispatches a Celery chord that renders any not-yet-ready `MixPlan`s in parallel, then stitches them into a single FLAC at `queue_mixes/<queue_id>.flac` (equal-power crossfade at each seam junction). The Player polls the resulting `QueueRender` row and switches to the stitched mix as the playback source when ready, falling back to per-song playback. Phase 8.5 lands the two Phase 9 prerequisites: Genius lyrics with `(×N)` repeat-marker expansion + Needleman-Wunsch DTW alignment against Whisper using Soundex phonetic backoff, and a vocal-safety service that returns per-song safe-cut intervals by cross-referencing the aligned-words list against the vocal stem's RMS + peak envelope. See [docs/the notes](docs/the notes) + [docs/the notes](docs/the notes). Prior phases: [Phase 7](docs/the notes), [Phase 6](docs/the notes), [Phase 5](docs/the notes), [Phase 4](docs/the notes), [Phase 3](docs/the notes), [Phase 2](docs/the notes), [Phase 1](docs/the notes).
