...
**Phase 9 complete** — LLM mix planning. `POST /api/mix_plans/{id}/render` now uses the `GeminiProvider` to request an AI-planned transition if `USE_LLM_PLANNER` is true. The LLM consumes `AnalysisBundle`, aligned lyrics, and vocal-safe regions to emit a `MixPlan` JSON with tool calls (crossfade, pitch shift, tempo ramp, etc.). It caches plans via `StorageBackend` to save token costs and falls back to a deterministic planner if the LLM fails. See [docs/the notes](docs/the notes). Prior phases: [Phase 8.5](docs/the notes), [Phase 8](docs/the notes), [Phase 7](docs/the notes), [Phase 6](docs/the notes), [Phase 5](docs/the notes), [Phase 4](docs/the notes), [Phase 3](docs/the notes), [Phase 2](docs/the notes), [Phase 1](docs/the notes).
